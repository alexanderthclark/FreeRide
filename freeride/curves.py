...
class BaseAffine:
    ...
    def __init__(
        ...
        elements : list of AffineElement, optional
            List of AffineElement objects. If provided, it will override
            `intercept` and `slope`. Default is None.
        inverse : bool, optional
            Indicates if the transformation should be inverted.
            Default is True.
        ...
        if elements is None:
            ...
            elements = [
                AffineElement(slope=m, intercept=b, inverse=inverse)
                for m, b in zipped
            ]
    ...
    def _has_perfectly_inelastic_segment(self):
        """Return ``True`` if any element is perfectly inelastic (infinite
        slope)."""
    ...
    def has_perfect_segment(self):
        """bool: Whether the curve contains a perfectly elastic or inelastic
        segment."""
        return (
            self.has_perfectly_elastic_segment
            or self.has_perfectly_inelastic_segment
        )
    ...
    def from_points(cls, xy_points):
        ...
        In the future, this might be extended to allow for three or more
        points.
        """
    ...
    def horizontal_shift(self, delta, inplace=True):
        new_elements = [
            e.horizontal_shift(delta, inplace=False) for e in self.elements
        ]
        if inplace:
    ...
    def vertical_shift(self, delta, inplace=True):
        new_elements = [
            e.vertical_shift(delta, inplace=False) for e in self.elements
        ]
        if inplace:
            self.__init__(elements=new_elements)
        else:
...
class Demand(Affine):
    ...
    def __init__(
        self, intercept=None, slope=None, elements=None, inverse=True
    ):
        """
        ...
        if self.has_perfectly_elastic_segment:
            warnings.warn(
                "Created perfectly elastic demand curve. "
                "Note: Due to current implementation limitations, this curve "
                "cannot be used with Equilibrium or combined with other "
                "curves. The economics are valid, but the software support is "
                "incomplete.",
                UserWarning,
            )
        ...
        for intercept in self.intercept:
            if intercept <= 0:
                raise Exception(
                    ...
                    "the price intercept should be positive."
                )
    ...
    def q(self, p):
        """
        Calculate quantity demanded at price p, handling perfectly elastic
        segments.
            ...
            for element in self.elements:
                if element.slope == 0:  # Horizontal segment
                    ...
                    if np.isclose(p, p_star):
                        warnings.warn(
                            f"Quantity demanded is indeterminate at P={p_star}"
                            " for perfectly elastic demand. "
                            "Returning np.inf as a placeholder "
                            "(actual quantity is indeterminate).",
                            UserWarning,
                        )
                        return np.inf
                    elif p > p_star:
...
class Supply(Affine):
    ...
    def __init__(
        ...
        if self.has_perfectly_elastic_segment:
            warnings.warn(
                "Created perfectly elastic supply curve. "
                "Note: Due to current implementation limitations, this curve "
                "cannot be used with Equilibrium or combined with other "
                "curves. The economics are valid, but the software support is "
                "incomplete.",
    ...
    def q(self, p):
        """
        Calculate quantity supplied at price p, handling perfectly elastic
        segments.
        ...
        if self.has_perfectly_elastic_segment:
            for element in self.elements:
                if element.slope == 0:  # Horizontal segment
                    ...
                    if np.isclose(p, p_star):
                        warnings.warn(
                            f"Quantity supplied is indeterminate at P={p_star}"
                            " for perfectly elastic supply. "
                            "Returning np.inf as a placeholder (actual "
...
class PPF(BaseAffine):
    ...
    def __init__(
        ...
        elements : list of AffineElement, optional
            A list of AffineElements whose horizontal sum defines the
            PPF.
        inverse : bool, optional
            When inverse is True, it is assumed that equations are in the form
            P(Q).
    ...
    def __call__(self, x):
        """Return the quantity of the second good for ``x`` units of the
        first."""
        for piece in self.pieces:
            if piece:
                a, b = piece._domain
    ...
    def plot(
        ...
        **kwargs,
        ...
        if backend == "bokeh":
            ...
            # Add HoverTool
            hover = HoverTool(
                tooltips=[("", "@label")], renderers=[p.renderers[-1]]
            )
            p.add_tools(hover)
            ...
            plot_dict = {
                key: kwargs[key] for key in kwargs if key in param_names
            }
            # Plot each element
            for piece in self.pieces:
                if piece:
