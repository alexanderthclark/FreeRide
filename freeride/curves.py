...
class Affine(BaseAffine):
    ...
    def __init__(self, intercept=None, slope=None, elements=None, inverse=True, sum_elements=True):
        ...
        if self.has_perfectly_elastic_segment:
            ...
            self.pieces = pieces
            # For non-summing mode, we need to determine the cuts from the domains
            # Each element should already have its domain set
            cuts = []
            for element in self.elements:
                if hasattr(element, '_domain') and element._domain:
                    cuts.extend(list(element._domain))
            cuts = sorted(list(set(cuts)))  # Remove duplicates and sort
            if not cuts:
                cuts = [0, np.inf]
            
            # Create sections based on the domains
            sections = []
            qsections = []
            for element in self.elements:
                if hasattr(element, '_domain') and element._domain:
                    ...
                    qsections.append((min(domain), max(domain)))  # Will be updated later
        ...
        if not self.has_perfectly_elastic_segment and self.sum_elements:
        ...
        if not self.sum_elements:
            # For non-summing mode, create conditions based on piece domains
            cond = []
            for section in sections:
                if section[1] == np.inf:
                    cond.append(f"q \\geq {section[0]}")
                else:
                    cond.append(f"{section[0]} \\leq q \\leq {section[1]}")
        else:
    ...
    def q(self, p):
        # returns q given p
        if not self.sum_elements:
            # In non-summing mode, find the piece that contains this price
            for piece in self.pieces:
                if piece:
                    # Check if p is in the price range for this piece
                    # This is a bit tricky because we need to consider the piece's domain
                    try:
                        q_val = piece.q(p)
